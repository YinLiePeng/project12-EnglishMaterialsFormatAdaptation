...
主要改进：
1. 表格区域文本保留：不过滤表格内的段落，而是将表格内容提取为段落
2. 行距计算改进：使用页面级统计，为所有段落计算行距
3. 对齐检测改进：基于页面宽度比例，更准确的居中/左对齐判断
4. 段落合并优化：更智能的合并策略，减少过度合并
5. 字体信息保留：更准确的字体映射和大小提取
"""
...
class PDFParser:
    ...
    def convert_to_content_elements(self) -> list:
        """将PDF内容转换为ContentElement列表（v4全面优化版）
        ...
        for page_num in range(len(self.doc)):
            ...
            for i in range(1, len(filtered_lines)):
                gap = filtered_lines[i]["y_position"] - filtered_lines[i-1]["y_end"]
                if gap > 0 and gap < 50:  # 合理的行距范围
                    gaps.append(gap)
            if gaps:
                gaps_sorted = sorted(gaps)
                page_line_gaps[page_num] = gaps_sorted[len(gaps_sorted) // 2]
        ...
        paragraphs = self._adaptive_merge_lines_v4(all_raw_lines)

        # 为所有段落计算行距（使用页面级统计）
        for para in paragraphs:
            page_num = para["page_num"]
            if page_num in page_line_gaps:
                font_sz = para["font"]["size"] if para.get("font") else 12
                median_gap = page_line_gaps[page_num]
                line_h = median_gap + font_sz
                ratio = line_h / font_sz if font_sz > 0 else 1
                if ratio > 0.8:
                    para["format"]["line_spacing"] = round(ratio, 2)
    ...
    def _adaptive_merge_lines_v4(self, raw_lines: list) -> list:
        """自适应段落合并（v4）
        ...
        for i in range(1, len(raw_lines)):
            ...
            if same_page and same_col:
                ...
                x_start_similar = abs(prev["x_start"] - curr["x_start"]) < 20
                ...
                if is_question_option or is_numbered or is_option:
                    ...
                    should_merge = True  # 重叠行必须合并
                elif y_gap < prev_fs * 0.1 and font_size_similar:
                    should_merge = True  # 极近的行合并
                elif font_size_similar and bold_same and x_start_similar:
                    # 只有在行距很小且文本连续时才合并
                    if y_gap < dominant_gap * 0.9:
                        if (
                            curr_text
    ...
    def _finalize_raw_line_group(self, group: list) -> dict:
        ...
        if page_rect and group:
            ...
            if all_x0 and all_x1:
                ...
                left_m = min_x0 - page_rect.x0
                right_m = page_rect.x1 - max_x1
                text_width = max_x1 - min_x0
                
                # 改进的居中检测
                center_threshold = page_w * 0.15
                if abs(left_m - right_m) < center_threshold:
                    if text_width < page_w * 0.85:
                        alignment = "center"
                # 改进的右对齐检测
                elif right_m > left_m * 1.5 and left_m > page_w * 0.2:
                    alignment = "right"
