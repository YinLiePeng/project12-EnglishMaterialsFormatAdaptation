...
async def get_task_status(task_id: str, db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "data": {
            ...
            "enable_cleaning": task.enable_cleaning,
            "enable_correction": task.enable_correction,
            "use_hybrid": task.use_hybrid,
        ...
        try:
            ...
            if file_ext == ".pdf":
                ...
                if detection_result.pdf_type.value == "scanned":
                    ...
                    yield _sse_event(
                        "progress",
                        {"stage": "ocr", "message": "检测到扫描版PDF，正在使用OCR识别..."},
                    )
                    from app.services.ocr.pipeline import process_scanned_pdf

                    elements = await process_scanned_pdf(
                    ...
                    if not elements:
                        yield _sse_event(
                            ...
                            {
                                "code": "OCR_FAILED",
                                "message": "扫描版PDF需要OCR识别，但OCR服务返回了空结果。请确认LLM API密钥配置正确。",
                            },
                        )
                        async with AsyncSessionLocal() as db:
                            result = await db.execute(
                            ...
                            if t:
                                t.status = "failed"
                                t.error_message = "OCR识别失败：LLM API密钥未配置或无效"
                                t.error_code = "OCR_FAILED"
                    ...
                    yield _sse_event(
                        "progress",
                        {"stage": "ocr", "message": "检测到混合型PDF，正在处理..."},
                    )
                    ...
                    elements = pdf_parser.convert_to_content_elements()
                    pdf_parser.close()
            else:
...
async def ai_recognize(
    ...
    # 重新解析文档
    try:
        file_ext = Path(task.input_file_path).suffix.lower()
        if file_ext == ".pdf":
            from app.services.pdf import PDFParser

            pdf_parser = PDFParser(task.input_file_path)
            elements = pdf_parser.convert_to_content_elements()
            pdf_parser.close()
        else:
